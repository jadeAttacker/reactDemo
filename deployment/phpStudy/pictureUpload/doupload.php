...
//4 对文件类型的过滤
if (!in_array($file["type"], $arrayList)) {
	die("类型不符合");
}
//5上传后的文件名定义
$fileinfo=pathinfo($file["name"]);//取得文件名（带后缀，即1timg.jpg），以数组的形式返回文件路径的信息。
do{
...
if(is_uploaded_file($file["tmp_name"])) {//判断是否是上传文件
	if (move_uploaded_file($file["tmp_name"], $path."/".$newfile)) {//移动到相应路径，叫什么名，都应该写上
		echo "上传成功！";
